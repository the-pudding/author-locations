...
d3.selection.prototype.puddingChartTimeline = function init(options) {
  function createChart(el) {
    ...
    function handleBookHover(d) {
      ...
      $tooltip.select("[data-js='tooltip__residence'] span").html(() => {
        ...
        const u = unique([].concat(...allMatches)).map(v => {
          return `<span class='inner__color' style='background-color:${scaleColor(
            v
          )}'></span> <span class='inner__loc'>${v}</span>`;
        });
